...
function ReaderAnnotationsPanel({
    ...
    return (
        <aside aria-label={tt('notesAndHighlights')} className="absolute top-4 right-4 bottom-4 z-30 flex w-[22rem] flex-col overflow-hidden rounded-2xl border backdrop-blur-xl shadow-2xl" style={{ backgroundColor: `${themeStyle.card}f2`, borderColor: themeStyle.border, color: themeStyle.text }}>
            <div className="shrink-0 px-4 py-3" style={{ borderBottom: `1px solid ${themeStyle.border}` }}>
                <div className="mb-3 flex items-center justify-between">
                    ...
                    <button type="button" onClick={onClose} aria-label={tt('closeAnnotations')} title={tt('closeAnnotations')} className="flex h-8 w-8 items-center justify-center rounded-lg transition-opacity hover:opacity-70" style={{ color: themeStyle.text }}>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
                    </button>
                </div>
                <div className="flex items-center gap-2 text-[11px]">
                    <button type="button" aria-pressed={filterValue === 'all'} onClick={() => setFilterValue('all')} className="rounded-full px-3 py-1 transition-opacity hover:opacity-80" style={filterChipStyle('all')}>{tt('all')}</button>
                    <button type="button" aria-pressed={filterValue === 'highlight'} onClick={() => setFilterValue('highlight')} className="rounded-full px-3 py-1 transition-opacity hover:opacity-80" style={filterChipStyle('highlight')}>{tt('highlights')}</button>
                    <button type="button" aria-pressed={filterValue === 'note'} onClick={() => setFilterValue('note')} className="rounded-full px-3 py-1 transition-opacity hover:opacity-80" style={filterChipStyle('note')}>{tt('notes')}</button>
                </div>
                <AnnotationExportControls bookId={bookId} annotationCount={annotations.length} loading={loading} tt={tt} compact />
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
                {loading ? (
                    <div role="status" aria-label={tt('loadingAnnotations')} className="flex justify-center py-10"><div className="h-7 w-7 animate-spin rounded-full border-2 border-current border-t-transparent opacity-50" /></div>
