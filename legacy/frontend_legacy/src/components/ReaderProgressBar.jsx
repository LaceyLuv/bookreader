...
function ReaderProgressBar({
    ...
    useEffect(() => {
        if (!isDragging) {
            setDraftProgress(normalizedProgress)
        }
    ...
    const previewPage = hasTotalPages
        ? clamp(Math.round(draftProgress * (totalPages - 1)) + 1, 1, totalPages)
        : '?'
    const previewPercent = Math.round(draftProgress * 100)
    const maxPageText = hasTotalPages ? totalPages : '?'

    return (
        <div className="shrink-0 px-4 py-2" style={{ borderTop: '1px solid var(--reader-border)' }}>
            <div className="mx-auto w-full max-w-4xl rounded-xl px-3 py-2.5"
                style={{ backgroundColor: 'color-mix(in srgb, var(--app-bg) 55%, transparent)' }}>
